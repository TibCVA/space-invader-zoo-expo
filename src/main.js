// Space Invader Zoo — UI SAFE iOS : clics fiables, import robuste, init verrouillée
...
function show(msg, ms=2000){ toast.textContent = msg; toast.classList.remove('hidden'); clearTimeout(show._t); show._t=setTimeout(()=>toast.classList.add('hidden'), ms); }
...
let initResolve; const initReady = new Promise(r=>initResolve=r);
...
(async function init3D(){
  try{
    ...
    camera.position.set(0,7.6,13); scene.add(camera);

    controls = new OrbitControls(camera, renderer.domElement);
    controls.enableDamping = true; controls.minDistance=6; controls.maxDistance=32;
    controls.autoRotate = true; controls.autoRotateSpeed = 0.45;

    const hemi = new THREE.HemisphereLight(0xbfd7ff, 0x202638, 0.65); scene.add(hemi);
    dirLight = new THREE.DirectionalLight(0xffffff, 1.05);
    dirLight.position.set(8,10,6); dirLight.castShadow = true;
    dirLight.shadow.mapSize.set(2048,2048); dirLight.shadow.bias=-0.00035; scene.add(dirLight);
    ...
    planet = createPlanetEXPO(4.0, THREE); scene.add(planet.group);

    // post-proc tolérant
    composer = new EffectComposer(renderer);
    composer.addPass(new RenderPass(scene,camera));
    try{
      if (!isIOS) composer.addPass(new SMAAPass(innerWidth,innerHeight));
      else {
    ...
    try{ composer.addPass(new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),0.42,0.9,0.8)); }catch{}

    addEventListener('resize', ()=>{
      camera.aspect=innerWidth/innerHeight; camera.updateProjectionMatrix();
      renderer.setSize(innerWidth,innerHeight); try{composer.setSize(innerWidth,innerHeight);}catch{}
    });
    ...
    function animate(){
      ...
      controls.update(); planet.update(dt); updateInvaders(dt); updateExpo(dt);
      try{ composer.render(); }catch(e){ renderer.render(scene,camera); }
      requestAnimationFrame(animate);
    }
    requestAnimationFrame(animate);
  }catch(e){
    show('⚠️ Problème d’initialisation WebGL'); console.warn(e);
  }finally{
    initResolve(); // scène déclarée prête (ou dégradée), pour ne jamais bloquer l’UI
    restoreState();
  }
})();

function updateLights(){
  if (!dirLight || !planet) return;
  dirLight.position.copy(planet.sunDir().multiplyScalar(12));
  dirLight.intensity = THREE.MathUtils.lerp(0.2,1.3, planet.sunElev());
}
...
fileInput.addEventListener('change', async ()=>{
  ...
  try{
    ...
    if (!files.length){ show('Aucune image sélectionnée'); return; }
    ...
    show('✅ Image importée');
  }catch(e){ show('⚠️ Import impossible'); console.warn(e); }
  fileInput.value='';
});
...
btnSamples.addEventListener('click', async ()=>{
  await ensureInit();
  try{
    const urls=['assets/samples/mars_56.jpg','assets/samples/mars_36.jpg','assets/samples/caz_32.jpg'];
    const url=urls[Math.floor(Math.random()*urls.length)];
    const img=await imageFromURL(url);
    await addInvader(img, url);
    show('✅ Exemple chargé');
  }catch(e){
    show('⚠️ Exemples introuvables (place-les dans assets/samples/)');
    console.warn(e);
  }
});
...
btnReset.addEventListener('click', async ()=>{
  ...
  invaders.length=0; saveState(); show('🗑 Scène vidée');
});

// -------- Invaders --------
async function addInvader(img, dataURL=null){
  if (!planet || !invadersGroup) return;
  const node = await buildInvaderFromImageEXPO(img, currentParams(), THREE);
  const box = new THREE.Box3().setFromObject(node), size=new THREE.Vector3(); box.getSize(size);
  node.scale.setScalar(1.9/Math.max(size.x,size.y,size.z));

  const holder=new THREE.Group(); holder.add(node); invadersGroup.add(holder);
...
function updateInvaders(dt){
  ...
  for (let i=0;i<invaders.length;i++){
    const a=invaders[i], pos=a.node.position.clone(), n=pos.clone().normalize();
    const R=planet.surfaceRadius(n)+0.34;
    const acc=new THREE.Vector3(); let aliC=0,cohC=0; const avgN=new THREE.Vector3(),avgP=new THREE.Vector3();
    for (let j=0;j<invaders.length;j++) if (i!==j){
      const b=invaders[j], d=pos.distanceTo(b.node.position);
      if (d<sep){
    ...
    if (aliC>0){ avgN.multiplyScalar(1/aliC);
      const tang=avgN.sub(n.clone().multiplyScalar(avgN.dot(n))).normalize(); acc.addScaledVector(tang,0.06);}
    if (cohC>0){ avgP.multiplyScalar(1/cohC);
      const to=avgP.sub(pos).normalize(); const tang=to.sub(n.clone().multiplyScalar(to.dot(n))).normalize(); acc.addScaledVector(tang,0.05);}
    const rand=new THREE.Vector3().randomDirection(); const randTan=rand.sub(n.clone().multiplyScalar(rand.dot(n))).normalize(); acc.addScaledVector(randTan,0.04);
    const grad=planet.gradient(n); if (grad.lengthSq()>0.0004){ const downhill=grad.negate().sub(n.clone().multiplyScalar(grad.negate().dot(n))).normalize(); acc.addScaledVector(downhill,0.06); }
    const heading=pos.clone().cross(n).normalize(); const vel=heading.add(acc.multiplyScalar(dt*0.0015)).normalize();
    const axis=new THREE.Vector3().crossVectors(pos,vel).normalize(); const ang=(a.speed*dt*0.12)/R;
    pos.applyAxisAngle(axis,ang).setLength(R); a.node.position.copy(pos);
    a.bob+=dt*0.0027; a.node.position.add(n.multiplyScalar(Math.sin(a.bob)*0.06));
...
addEventListener('pointerdown', async (e)=>{
  ...
  if (hits.length){ const p=hits[0].object.getWorldPosition(new THREE.Vector3()); controls.target.lerp(p,0.9); }
});

// Persistance
const LS='sizo-expo-v7';
function saveState(){ try{ const inv=invaders.map(i=>({dataURL:i.dataURL??null})); localStorage.setItem(LS, JSON.stringify({inv})); }catch{} }
async function restoreState(){
  try{
    const txt=localStorage.getItem(LS); if(!txt) return;
    const st=JSON.parse(txt);
    for (const it of st.inv??[]){ if(!it.dataURL) continue; const img=await imageFromDataURL(it.dataURL); await addInvader(img,it.dataURL); }
...
function updateExpo(dt){
  ...
  const zoom=parseFloat(expoZoom.value); const angle=expoTimer*0.4; const elev=Math.sin(expoTimer*0.7)*0.3+0.5;
...
window.addEventListener('error', (e)=>{ show('⚠️ Erreur JS : voir console'); console.warn(e.error||e.message); });