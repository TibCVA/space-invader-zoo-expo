...
| Plan de masse de la cité — tout construit couvre toutes les terrasses (exigence du propriétaire, lot 2.5-cité) | **fait** | plan recomposé en cinq bandes logiques, échelles 130-250, terrasses par faction ET par orientation (`masse.ts`), module portrait ×1,8, porte portrait ; chaînes empilées, améliorations fusionnées à leur demeure ; couverture mesurée 80,5-92,2 % (cible ≥ 70), test aux cadres réels bureau/iPhone éprouvé en le défaisant (8/12 rouges sur l'ancien plan) ; captures shots/masse2 vérifiées à l'œil, zéro erreur console — reste du lot 2.5 : pincer-zoomer cité et combat sur iPhone |
...
| Combat — câblage (lot 2.3-2.4) | **fait, en partie** | école des sorts lue au lieu d'être devinée (18/32 fautes éliminées) ; impact calé sur le contact du geste (117 ms d'avance supprimées, temps du geste dans le gréement, source unique) ; épée/flèche sur la case de départ. Restent : un effet visuel par école, projectiles par tireur, cadavres, densité d'obstacles |
| Téléphone — la scène perdait 90 pixels | **corrigé pour de bon le 20/08 au soir** | Le premier correctif était INOPÉRANT et son raisonnement faux : il retirait la réserve par un `padding-bottom` sur `.jeu-scene`, or `.jeu-scene__toile` est en position absolue et son bloc conteneur EST la boîte de rembourrage — `inset:0` s'étale par-dessus le padding, qui n'enlève donc rien. Mesuré sur deux captures iPhone : les six boutons d'action étaient dessinés à 786-838 pt sur un écran de 784, entièrement sous la barre de pouce, et invisibles. Une variable `--hmm-reserve-pouce` retranchée du `bottom` de l'élément absolu le corrige, et un test de géométrie (`battle/pouce.test.ts`, `battle/actions.test.ts`) garde l'invariant sur 375, 390 et 360 de large. Deux défauts de plus trouvés en mesurant : la rangée de six boutons débordait de 9 pt sur un iPhone SE et de 24 pt sur un écran de 360 ; et le haut des boutons recouvrait de 10 pt la bande de bascule du panneau |
| Pincer-zoomer cité et combat (exigence du propriétaire) | **fait** | module `pincement.ts` partagé, geste à un doigt laissé au jeu, butée sans glissement ; 10 tests |
| Hautes-chaumes et tourbières (lot 1.8, part 1) | **fait** | prairie+forêt 82,3 % → 74,5 %, lande 7,4 %, tourbières 5,1 % ; densité rendue au vert (851 objets, glaneur 2,8/j) ; teinte corrigée sur capture (l'orange fluo est devenu de l'herbe sèche) |
